...
import ServiceFormInline from "@/components/ServiceFormInline";
...
export default function ServicesClient({
    ...
    initialContent: { title: string; description: string } | null;
}) {
    const [allServices, setAllServices] = useState<Service[]>(initialServices);
    const [pageContent, setPageContent] = useState<{ title: string; description: string } | null>(initialContent);
    const [loading, setLoading] = useState(false); // No initial load delay
    ...
    const displayed = useMemo(() => {
        ...
        if (categories.length) list = list.filter((s) => categories.includes(s.category.toLowerCase()));
        list.sort((a, b) => {
                    ...
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 md:px-8 pt-10 sm:pt-12 pb-12 md:pb-16">
                        ...
                        <div className="mb-8 flex flex-col items-center justify-center sm:mb-10 text-center">
                            ...
                            <p className="mt-2 text-sm sm:text-base text-[var(--muted)]">
                                Explore our full range of interior and exterior design services.
                            </p>
                            {isAdmin && (
                                <button
                                    onClick={() => {
                            ...
                            <div className="relative" ref={sortRef}>
                                ...
                                {sortOpen && (
                                    <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-52 rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-2xl shadow-black/10 overflow-hidden">
                                        <p className="px-4 pt-3 pb-2 text-[10px] font-black uppercase tracking-widest text-[var(--muted)]">
                                            Sort by
                                        </p>
                                        {SORT_OPTIONS.map((opt) => (
                                            <button
                                                ...
                                                className={`w-full flex items-center justify-between px-4 py-2.5 text-sm font-medium transition-colors ${sortBy === opt.value
                        ...
                        {!loading && (
                            <p className="text-xs font-medium text-[var(--muted)] mb-5">
                                ...
                                {activeFilterCount > 0 && (
                                    <button onClick={clearFilters} className="ml-3 text-blue-500 hover:underline">
                                        Clear filters
                                    </button>
                                )}
                            </p>
                            ...
                            <div className="grid gap-6 sm:gap-8 sm:grid-cols-2 lg:grid-cols-3">
                                {displayed.map((s) => (
                                    <Link
                                        ...
                                        className="group relative rounded-[2rem] p-3 sm:p-4 bg-[var(--card)] border border-[var(--border)] shadow-sm hover:shadow-xl hover:border-blue-500/30 transition-all duration-300 block overflow-hidden"
                                    >
                                        {isAdmin && (
                                                ...
                                                <button
                                                    onClick={async (e) => {
                                                        ...
                                                        if (confirm(`Delete service ${s.name}?`)) {
                                                            const r = await fetch(`/api/services?id=${s.id}`, { method: "DELETE" });
                                                            if (r.ok) setAllServices((p) => p.filter((x) => x.id !== s.id));
                                                        }
                                                    }}
                                        ...
                                        <div className="relative aspect-[4/3] overflow-hidden rounded-[1.5rem] sm:rounded-[1.75rem] mb-5 sm:mb-6 bg-[var(--muted-bg)]">
                                            ...
                                            <div className="absolute top-3 right-3 bg-black/40 backdrop-blur-md px-2.5 py-1 rounded-lg border border-white/20 shadow-sm">
                                                <span className="text-[10px] sm:text-xs font-medium text-white tracking-widest uppercase">
                                                    {s.timelineWeeks.min}-{s.timelineWeeks.max} Wks
                                                </span>
                                            </div>
                                        </div>
                                        <div className="px-2 pb-2">
                                            ...
                                            <h2 className="font-heading text-xl md:text-2xl font-bold text-[var(--foreground)] mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                                {s.name}
                                            </h2>
                ...
                <div className="flex items-center justify-between mb-3">
                    ...
                    {categories.length > 0 && (
                        <button
                            ...
                            className="text-xs text-[var(--muted)] hover:text-red-500 transition-colors flex items-center gap-0.5"
                        >
