...
export default function ProjectsClient({
    ...
    initialServicesMap: Record<string, string>;
}) {
    const [allProjects, setAllProjects] = useState<Project[]>(initialProjects);
    const [servicesMap, setServicesMap] = useState<Record<string, string>>(initialServicesMap);
    const [loading, setLoading] = useState(false); // No initial load time!
                    ...
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 md:px-8 pt-10 sm:pt-12 pb-12 md:pb-16">
                        ...
                        <div className="mb-8 flex flex-col items-center justify-center sm:mb-10 text-center">
                            ...
                            <p className="mt-2 text-sm sm:text-base text-[var(--muted)]">
                                Browse our portfolio of completed and active design projects.
                            </p>
                            {isAdmin && (
                                <button
                                    onClick={() => {
                            ...
                            <div className="grid gap-6 sm:gap-8 sm:grid-cols-2 lg:grid-cols-3">
                                {displayed.map((p) => (
                                    <Link
                                        key={p.id}
                                        href={`/projects/${p.id}`}
                                        className="group relative rounded-3xl overflow-hidden border border-[var(--border)] bg-[var(--card)] aspect-[4/3] sm:aspect-[4/5] md:aspect-[3/4] shadow-sm block"
                                    >
                                        {isAdmin && (
                                        ...
                                        <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                                            <span className="text-white/80 font-medium text-[10px] uppercase tracking-widest bg-white/10 px-2 py-1 rounded backdrop-blur-md border border-white/10 w-max mb-3">
                                                {p.status}
                                            </span>
                                            ...
                                            <div className="mt-4 pt-4 border-t border-white/20 flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100 h-0 overflow-hidden group-hover:h-auto">
                                                <div className="flex justify-between text-xs font-medium">
                                                    ...
                                                    <span className="text-white">₹{p.budget.toLocaleString()}</span>
                                                </div>
                                                <div className="flex justify-between text-xs font-medium">
                                                    <span className="text-white/60">Timeline</span>
                                                    <span className="text-white">
                                                        {p.endDate ? `Finished ${p.endDate}` : `Started ${p.startDate}`}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div className="mt-14 text-center">
                            <Link
                                href="/contact"
                                className="inline-flex items-center justify-center rounded-lg bg-blue-600 px-6 py-3 text-[15px] font-semibold text-white hover:bg-blue-700 shadow-md transition-all"
                            >
                                Start your project
                            </Link>
                        </div>
