...
export default function ServicesPage() {
  ...
  useEffect(() => {
    fetch("/api/auth/sessions").then(res => {
        ...
        <>
          <div className="mx-auto max-w-7xl px-4 sm:px-6 md:px-8 pt-10 sm:pt-12 pb-12 md:pb-16">
            ...
            <div className="mb-8 flex flex-col items-center justify-center sm:mb-10 text-center">
              ...
              {isAdmin && (
                <button
                  onClick={() => { setServiceToEdit(null); setModalOpen(true); }}
                  ...
                  <Link key={s.id} href={`/services/${s.slug}`} className="group relative rounded-[2rem] p-3 sm:p-4 bg-[var(--card)] border border-[var(--border)] shadow-sm hover:shadow-xl hover:border-blue-500/30 transition-all duration-300 block overflow-hidden">
                    {isAdmin && (
                      <div className="absolute top-6 right-6 z-20 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={(e) => { e.preventDefault(); setServiceToEdit(s); setModalOpen(true); }}
                    ...
                    <div className="relative aspect-[4/3] overflow-hidden rounded-[1.5rem] sm:rounded-[1.75rem] mb-5 sm:mb-6 bg-[var(--muted-bg)]">
                      <Image
                        src={s.image && (s.image.startsWith('http') || s.image.startsWith('/')) ? s.image : 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?q=80&w=2000&auto=format&fit=crop'}
                        alt={s.name}
                        fill
